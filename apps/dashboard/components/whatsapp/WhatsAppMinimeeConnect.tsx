...
import { Loader2, Bot, CheckCircle2, RefreshCw, QrCode, AlertCircle, Phone, Building2 } from "lucide-react";
...
export function WhatsAppMinimeeConnect() {
  ...
  const checkStatus = async () => {
    try {
      const result = await api.getMinimeeWhatsAppStatus();
      setStatus(result);
      
      // Fetch account info if connected
      if (result.connected) {
        try {
          const info = await api.getMinimeeWhatsAppInfo();
          setAccountInfo(info);
        } catch (error) {
      ...
      toast.error(error instanceof Error ? error.message : "Failed to check Minimee WhatsApp status");
      setStatus({
        status: "error",
        running: false,
        connected: false,
  ...
  const fetchQRCode = async () => {
    try {
      const result = await api.getMinimeeWhatsAppQR();
  ...
  const handleRestart = async () => {
    ...
    try {
      await api.restartMinimeeWhatsAppBridge();
      toast.success("Minimee WhatsApp bridge restarted. Checking status...");
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span>Minimee WhatsApp Status</span>
          </div>
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <Bot className="h-5 w-5" />
          <span>Minimee WhatsApp Account</span>
        </div>
        {getStatusBadge()}
      </div>

      <div className="text-sm text-muted-foreground">
        Connect Minimee's dedicated WhatsApp account. This is Minimee's identity - you can chat with Minimee directly on WhatsApp, and messages will be synchronized with the dashboard.
      </div>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-blue-50 dark:bg-blue-950 p-4 rounded-lg space-y-3">
            <p className="text-sm font-semibold text-blue-900 dark:text-blue-100">
              Comment connecter le compte WhatsApp de Minimee :
            </p>
            <ol className="text-sm space-y-2 text-blue-800 dark:text-blue-200">
              <li className="flex items-start gap-2">
                <span className="font-bold text-blue-600 dark:text-blue-400">1.</span>
                <span>Ouvrez WhatsApp sur le téléphone dédié à Minimee</span>
              </li>
              <li className="flex items-start gap-2">
              ...
              <li className="flex items-start gap-2">
                ...
                <span>Scannez le QR code ci-dessus avec le téléphone</span>
              </li>
            </ol>
          </div>
        </div>
